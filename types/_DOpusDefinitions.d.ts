...
interface DOpusBlob {
   ...
   */
  copyFrom(
    sourceOrString?: DOpusBlob | string,
    toOrType?: number | string,
    from?: number,
    size?: number
  ): void;
 ...
 */
type DOpusCustomFieldData = Record<string, string | boolean | number | DOpusVector<any> | undefined>;
...
interface DOpusDialog {
  ...
  open(
    title?: string,
    defaultValue?: string,
    windowOrFileExt?: DOpusLister | DOpusTab | string
  ): DOpusItem;
...
interface DOpusFactoryConstructor {
   ...
   */
  vector(...elements: any): DOpusVector<any>;
}
...
interface DOpusFile extends String {
   ...
   */
  write(
    sourceStringOrBlob?: string | DOpusBlob,
    from?: number,
    size?: number
  ): number;
}
...
type DOpusFiletypeGroupWithoutMatchExt = new () => {
  [P in Exclude<keyof DOpusFiletypeGroup, "matchExt">]: DOpusFiletypeGroup[P];
};
...
interface DOpusFolderEnum {
   ...
   */
  next(count?: number, vector?: DOpusVector<any>): DOpusItem | DOpusVector;
}
...
interface DOpusGetCustomFieldData {
  ...
  field_labels(name: string): string;
}
...
type DOpusListerWithoutLastActive = new () => {
  [P in Exclude<keyof DOpusLister, "lastActive">]: DOpusLister[P];
};
/**
...
interface DOpusTab {
  ...
  updateFAYTSuggestions(
    params?: [string] | DOpusVector<string> | DOpusMap
  ): void;
}
...
interface DOpusTabGroup {
   ...
   */
  link(
    obj1?: DOpusTabGroupTabEntry,
    obj2?: DOpusTabGroupTabEntry,
    type?: string
  ): void;
...
interface DOpusTabGroupTabList {
   ...
   */
  addTab(
    TabGroupTabEntry?: object,
    path?: string,
    name?: string
  ): DOpusTabGroupTabEntry;
...
type DOpusVarWithoutDelete = new () => {
  [P in Exclude<keyof DOpusVar, "delete">]: DOpusVar[P];
};
/**
...
type DOpusViewerWithoutLastActive = new () => {
  [P in Exclude<keyof DOpusViewer, "lastActive">]: DOpusViewer[P];
};
/**
...
type StringWithoutMatch = new () => {
  [P in Exclude<keyof String, "match">]: String[P];
};
